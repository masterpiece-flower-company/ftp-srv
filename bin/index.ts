...
function setupState(_args: any) {
  ...
  function setupCredentials() {
    ...
    const setCredentials = (
      ...
      _state.credentials[username] = { password, root };
    };
...
function startFtpServer(_state: Record<string, any>) {
  for (const key in _state) {
    if (_state[key] === undefined) delete _state[key];
  }

  function checkLogin(data: any, resolve: any, reject: any) {
    ...
    if (
      ...
      return resolve({
        root: (user && user.root) || _state.root,
      });
    }
